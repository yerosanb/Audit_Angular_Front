...
import { ControllableExpense } from 'app/branch_audit/model/ControllableExpense/controllable-expense';
...
@Component({
  selector: 'app-controllable-reviewer-pending',
  templateUrl: './controllable-reviewer-pending.component.html',
  styles: [
    `
  ...
  styleUrls: ['./controllable-reviewer-pending.component.css'],
})
export class ControllableReviewerPendingComponent {
  ...
  controllableExpense = new ControllableExpense();
  updated = false;
  submitted = false;
  errorMessage: String = '';
  loading = true;
  ...
  ngOnInit(): void {
    ...
    this.getPendingFindings(this.user_type);
  }

  getPendingFindings(user_type: User) {
    this.controllableReviewerService.getPendingAudits(user_type).subscribe(
      (response) => {
  ...
  onDataLoad(data: any) {
    if (data[1]) {
      ...
      this.getPendingFindings(auditor_id);
