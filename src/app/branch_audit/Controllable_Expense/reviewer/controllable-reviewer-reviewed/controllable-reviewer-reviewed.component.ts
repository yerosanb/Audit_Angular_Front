...
import { ControllableReviewerService } from '../service/controllable-reviewer.service';
@Component({
  selector: 'app-controllable-reviewer-reviewed',
  templateUrl: './controllable-reviewer-reviewed.component.html',
  styles: [
    `
  ...
  styleUrls: ['./controllable-reviewer-reviewed.component.css'],
})
export class ControllableReviewerReviewedComponent {
  ...
  constructor(
    ...
    private controllableReviewerService: ControllableReviewerService,
  ...
  ngOnInit(): void {
    ...
    this.controllableReviewerService.getReviewedFindings(user.id).subscribe(
      (response) => {
  ...
  compileAudits() {
    let selected_audits_expense_type: String[] = [];
    if (this.selectedAudits) {
        this.selectedAudits.map(audit => {
          if (audit.controllableExpense) {
            if (audit.controllableExpense.controllableExpenseType) {
              audit.controllableExpense.controllableExpenseType.map(expense => {
                if (expense.name) {
                  selected_audits_expense_type.push(expense.name);
                }
              });
            }
          }
    ...
    if (selected_audits_expense_type) {
      if (this.areAllEqual(selected_audits_expense_type)) {
        this.outputData.push(this.selectedAudits);
        this.outputData.push('ControllableExpense');
        this.compile_ui = true;
      } else {
        this.messageService.add({
          severity: 'error',
          summary:
            'Please select audits with the same controllable expense type',
          detail: '',
        });
      }
    }
