...
@Component({
  selector: 'app-controllable-expense-pending',
  templateUrl: './controllable-expense-pending.component.html',
  styles: [
    `
  ...
  styleUrls: ['./controllable-expense-pending.component.css'],
})
export class ControllableExpensePendingComponent {
  ...
  ngOnInit() {
    ...
    this.cols = [
      ...
      {
        field: 'controllable_expense_type',
        header: 'Controllable Expense Type',
      },
  ...
  getFindings(auditor_id: number) {
    ...
    this.ControllableExpenseService.getDraftedSuspenseAccount(
      auditor_id
    ).subscribe(
      (response) => {
  ...
  deleteAudit(audit: BranchFinancialAudit) {
    this.confirmationService.confirm({
      ...
      accept: () => {
        this.ControllableExpenseService.deleteSuspenseAccount(audit).subscribe({
          next: (response) => {
  ...
  passAudit(audit: BranchFinancialAudit) {
    this.confirmationService.confirm({
      ...
      icon: 'pi pi-exclamation-triangle',
      accept: () => {
        this.ControllableExpenseService.passSuspenseAccount(audit).subscribe({
          next: (response) => {
            this.audits = this.audits.filter((val) => val.id !== audit.id);
            this.audit = new BranchFinancialAudit();
            this.messageService.add({
  ...
  deleteSelectedAudits() {
    this.confirmationService.confirm({
      ...
      icon: 'pi pi-exclamation-triangle',
      accept: () => {
        this.ControllableExpenseService.deleteSelectedSuspenseAccount(
          this.selectedAudits
        ).subscribe({
          next: (response) => {
  ...
  passSelectedAudits() {
    ...
    this.confirmationService.confirm({
      ...
      icon: 'pi pi-exclamation-triangle',
      accept: () => {
        this.ControllableExpenseService.passSelectedSuspenseAccount(
          this.selectedAudits
        ).subscribe({
          next: (response) => {
            this.audits = this.audits.filter(
