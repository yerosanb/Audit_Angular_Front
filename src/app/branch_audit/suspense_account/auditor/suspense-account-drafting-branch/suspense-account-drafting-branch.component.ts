...
import { SuspenseAccountBranchService } from '../service/suspense-account-branch.service';
import { SuspenseAccountTypeService } from '../service/suspense-account-type.service';

@Component({
  selector: 'app-suspense-account-drafting-branch',
  templateUrl: './suspense-account-drafting-branch.component.html',
  styles: [
    `
  ...
  styleUrls: ['./suspense-account-drafting-branch.component.css'],
})
export class SuspenseAccountDraftingBranchComponent {
  ...
  selectedCities3: any[];
  suspenseAccountDataOptions: SuspenseAccountType[];
  ...
  constructor(
    ...
    private suspenseAccountBranchService: SuspenseAccountBranchService,
  ...
  ngOnInit() {
    ...
    this.cols = [
      ...
      { field: 'balance_per_tracer', header: 'Balance Per Tracer' },
      {
        field: 'balance_per_trial_balance',
        header: 'Balance Per Trial Balance',
      },
      { field: 'difference', header: 'Difference' },
      { field: 'suspense_account_type', header: 'Suspense Account Type' },
  ...
  getFindings(auditor_id: number) {
    this.suspenseAccountBranchService.getDraftedSuspenseAccount(auditor_id).subscribe(
      (response) => {
  ...
  deleteAudit(audit: BranchFinancialAudit) {
    this.confirmationService.confirm({
      ...
      accept: () => {
        this.suspenseAccountBranchService
          .deleteSuspenseAccount(audit)
          .subscribe({
            next: (response) => {
  ...
  passAudit(audit: BranchFinancialAudit) {
    this.confirmationService.confirm({
      ...
      icon: 'pi pi-exclamation-triangle',
      accept: () => {
        this.suspenseAccountBranchService.passSuspenseAccount(audit).subscribe({
          next: (response) => {
            this.audits = this.audits.filter((val) => val.id !== audit.id);
            this.audit = new BranchFinancialAudit();
            this.messageService.add({
  ...
  deleteSelectedAudits() {
    this.confirmationService.confirm({
      ...
      accept: () => {
        this.suspenseAccountBranchService
          .deleteSelectedSuspenseAccount(this.selectedAudits)
          .subscribe({
            next: (response) => {
  ...
  passSelectedAudits() {
    ...
    this.confirmationService.confirm({
      ...
      accept: () => {
        this.suspenseAccountBranchService
          .passSelectedSuspenseAccount(this.selectedAudits)
          .subscribe({
            next: (response) => {
              this.audits = this.audits.filter(
                (val) => !this.selectedAudits.includes(val)
  ...
  getSuspenseAccountType() {
    this.suspenseAccountTypeService.getSuspenseAccountType().subscribe(
      (response) => {
        this.suspenseAccountDataOptions = response;
        //console.log(this.audits);
      },
      (error) => (error: HttpErrorResponse) => {
        this.messageService.add({
