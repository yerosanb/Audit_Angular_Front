...
import { SuspenseAccountReviewerService } from '../../service/suspense-account-reviewer.service';
  ...
  @Component({
    selector: 'app-reviewed-suspense-account-reviewer',
    templateUrl: './reviewed-suspense-account-reviewer.component.html',
    styles: [
      `
    ...
    styleUrls: ['./reviewed-suspense-account-reviewer.component.css']
  })
  export class ReviewedSuspenseAccountReviewerComponent {
  ...
  getFindings(auditor_id: number) {
    this.suspenseAccountReviewerService
      .getReviewedSuspenseAccount(auditor_id)
      .subscribe(
        (response) => {
  ...
  unReviewSuspenseAccount(audit: BranchFinancialAudit) {
  ...
  unReviewSelectedSuspenseAccount() {
    this.confirmationService.confirm({
  ...
  //   this.outputData.push('SuspenseAccount');
  //   this.compile_ui = true;
  // }
  compileAudits() {
    let selected_audits_account_type: String[] = [];
    if (this.selectedAudits) {
        this.selectedAudits.map(audit => {
          if (audit.suspenseAccountBranch) {
            if (audit.suspenseAccountBranch.suspenseAccountType) {
              audit.suspenseAccountBranch.suspenseAccountType.map(suspense => {
                if (suspense.name) {
                  selected_audits_account_type.push(suspense.name);
                }
              });
            }
          }
    ...
    if (selected_audits_account_type) {
      if (this.areAllEqual(selected_audits_account_type)) {
        this.outputData.push(this.selectedAudits);
        this.outputData.push('SuspenseAccount');
        this.compile_ui = true;
      } else {
        this.messageService.add({
          severity: 'error',
          summary: 'Please select audits with the same suspense account type',
          detail: '',
        });
      }
    }
