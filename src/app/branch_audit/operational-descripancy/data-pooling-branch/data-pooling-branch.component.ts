import { filter } from 'rxjs/operators';
import { ExportExcelService } from 'app/services/shared/export-excel.service';
import { HttpErrorResponse } from '@angular/common/http';
import { Component, Input } from '@angular/core';
import { StorageService } from 'app/services/shared/storage.service';
import ClassicEditor from '@ckeditor/ckeditor5-build-classic';
import { User } from 'app/models/admin/user';
import { ConfirmationService, MessageService } from 'primeng/api';
import * as FileSaver from 'file-saver';
import { environment } from 'environments/environment';
import { Observable } from 'rxjs';
import { BranchFinancialAudit } from 'app/branch_audit/model/branch-financial-audit';
import { BranchFinancialAuditService } from 'app/branch_audit/services/branch-financial-audit.service';
import { CommonReviewerBranchFinancialService } from 'app/branch_audit/regional_compiled_audits/service/common-reviewer-branch-financial.service';
import { DataPoolingServiceBranchService } from 'app/branch_audit/common/service/data-pooling-service-branch.service';
import { OperationalDescripancyPooledData } from './operational-descripancy-pooled-data';
import { DateFormat } from 'app/models/shared/date-format';

interface ExportColumn {
  title: string;
  dataKey: string;
}
@Component({
  selector: 'app-data-pooling-branch',
  templateUrl: './data-pooling-branch.component.html',
  styles: [
    `
      :host ::ng-deep .p-dialog .audit-image {
        width: 150px;
        margin: 0 auto 2rem auto;
        display: block;
      }

      :host ::ng-deep .row_detail .ck-editor {
        width: 95%;
      }
      :host ::ng-deep .table_editor .ck-editor {
        width: 95%;
      }
    `,
  ],

  styleUrls: ['./data-pooling-branch.component.css'],
})
export class DataPoolingBranchComponent {
  environment = environment;

  public Editor = ClassicEditor;

  audit = new BranchFinancialAudit();
  audit_review: BranchFinancialAudit = new BranchFinancialAudit();
  audit_edit_history: BranchFinancialAudit[];

  updated = false;
  submitted = false;
  errorMessage: String = '';
  loading = true;

  audits: BranchFinancialAudit[] = [];

  selectedAudits: BranchFinancialAudit[] = [];
  edit_auditee: Boolean = false;
  reviewer: User = new User();
  activeIndex1: number = 0;
  activeState: boolean[] = [true, false, false];
  cols: any[];

  editHistoryDialog: Boolean = false;

  fileInfos: Observable<any>;

  category = '';

  audits_to_be_compiled: any[] = [];

  outputBranchFinancialAudit: BranchFinancialAudit[] = [];
  auditeeResponseDialog = false;

  audits_rectification: BranchFinancialAudit[] = [];
  audit_rectification = new BranchFinancialAudit();
  reciever = new User();

  _selectedColumns: any[];

  pooler = new OperationalDescripancyPooledData();

  selectedAuditReport: BranchFinancialAudit[];
  audits_to_be_exported: BranchFinancialAudit[];

  exportColumns!: ExportColumn[];

  selectedTitleForReport: string =
    'AFRFMS - Pooled Transaction Related Descripancies Report';

  dateFormat = new DateFormat();

  cols_export: any[];

  constructor(
    private storageService: StorageService,
    private fileService: BranchFinancialAuditService,
    private messageService: MessageService,
    private exportService: ExportExcelService,
    private poolingService: DataPoolingServiceBranchService,
    private confirmationService: ConfirmationService,
    private commonService: CommonReviewerBranchFinancialService
  ) {}

  ngOnInit(): void {
    let user = this.storageService.getUser();
    this.pooler.pooler = user.id;
    this.poolingService.getPooledData(this.pooler).subscribe(
      (response) => {
        this.audits = response;
        const uniqueCaseNumbers = new Set<String>();
        const realAudits: BranchFinancialAudit[] = [];

        for (const audit of this.audits) {
          if (audit.case_number && !uniqueCaseNumbers.has(audit.case_number)) {
            uniqueCaseNumbers.add(audit.case_number);
            realAudits.push(audit);
          }
        }
        this.audits = realAudits;
        this.loading = false;
      },

      (error) => (error: HttpErrorResponse) => {
        this.errorMessage = error.error.message;
        this.messageService.add({
          severity: 'error',
          summary:
            error.status == 401
              ? 'You are not permitted to perform this action!'
              : 'Something went wrong while fetching findings!',
          detail: '',
        });
      }
    );

    this.cols = [
      // { field: 'branch_name', header: 'Branch Name' },
      // { field: 'audit_case_number', header: 'Case Number' },
      { field: 'cash_type', header: 'Currency Type' },
      { field: 'operational_category', header: 'Category' },
      { field: 'account_number', header: 'Account Number' },
      { field: 'acount_holder_name', header: 'Account Holder Name' },
      { field: 'amount', header: 'Amount' },
      { field: 'transaction_date', header: 'Reporting Date' },
      { field: 'finding_date', header: 'Date of Discrepancy' },
      // { field: 'rectification_status', header: 'Rectification Status' },
      { field: 'rectification_date', header: 'Rectification Date' },
      // { field: 'audit_finding', header: 'Finding' },
      // { field: 'audit_impact', header: 'Impact' },
      // { field: 'audit_recommendation', header: 'Recommendation'},
      // { field: 'finding_detail', header: 'Finding Detail'}
    ];

    this.cols_export = [
      { field: 'branch_name', header: 'Branch Name' },
      { field: 'audit_case_number', header: 'Case Number' },
      { field: 'cash_type', header: 'Currency Type' },
      { field: 'operational_category', header: 'Category' },
      { field: 'account_number', header: 'Account Number' },
      { field: 'acount_holder_name', header: 'Account Holder Name' },
      { field: 'amount', header: 'Amount' },
      { field: 'transaction_date', header: 'Reporting Date' },
      { field: 'finding_date', header: 'Date of Discrepancy' },
      { field: 'finding', header: 'Finding' },
      { field: 'impact', header: 'Impact' },
      { field: 'recommendation', header: 'Recommendation' },
    ];

    this._selectedColumns = this.cols;
  }

  @Input() get selectedColumns(): any[] {
    return this._selectedColumns;
  }

  set selectedColumns(val: any[]) {
    //restore original order
    this._selectedColumns = this.cols.filter((col) => val.includes(col));
  }

  isString(value: any): boolean {
    return typeof value === 'string';
  }

  isNumber(value: any): boolean {
    return typeof value === 'number';
  }

  isBoolean(value: any): boolean {
    return typeof value === 'boolean';
  }

  isDate(value: any): boolean {
    return value instanceof Date;
  }
  //containsAnyField function checks if any of the fields in the fieldsToCheck array are present in the cols array. It returns true if any field is present, and false otherwise
  containsAnyField(array: any[], fields: string[]): boolean {
    return fields.some((field) => array.some((item) => item.field === field));
  }

  toggle(index: number) {
    this.activeState[index] = !this.activeState[index];
  }

  addAuditeeResponse(audit: BranchFinancialAudit) {
    this.audit = audit;
    this.outputBranchFinancialAudit = [];
    this.outputBranchFinancialAudit.push(this.audit);
    this.auditeeResponseDialog = true;
  }

  onDataChangeAuditeeResponse(data: BranchFinancialAudit[]) {
    if (data) {
      for (const dt of data) {
        if (dt.id) this.audits[this.findIndexById(dt.id)] = dt;
      }
    }
    this.auditeeResponseDialog = false;
  }

  deletePooledSelectedFindings() {
    this.reciever.id = this.storageService.getUser().id;
    this.selectedAudits[0].reciever = this.reciever;
    this.confirmationService.confirm({
      message: 'Are you sure you want to move selected audits to trash ?',
      header: 'Confirm',
      icon: 'pi pi-exclamation-triangle',
      accept: () => {
        this.poolingService.deletePooledData(this.selectedAudits).subscribe({
          next: (response) => {
            this.audits = this.audits.filter(
              (val) => !this.selectedAudits.includes(val)
            );
            this.selectedAudits = [];
            this.messageService.add({
              severity: 'success',
              summary: 'Successful',
              detail: 'Audits moved to trash',
              life: 3000,
            });
          },
          error: (error: HttpErrorResponse) => {
            this.loading = false;
            this.errorMessage = error.message;
            this.submitted = true;
            this.messageService.add({
              severity: 'error',
              summary:
                error.status == 401
                  ? 'You are not permitted to perform this action!'
                  : 'Something went wrong while moving findings to trash!',
              detail: '',
            });
          },
        });
      },
    });
  }

  findIndexById(id: number): number {
    let index = -1;
    for (let i = 0; i < this.audits.length; i++) {
      if (this.audits[i].id === id) {
        index = i;
        break;
      }
    }
    return index;
  }

  onDataChange(data: any) {
    this.audit.file_urls = data;
  }

  getFileUrls(audit: BranchFinancialAudit) {
    if (audit.file_urls) {
      this.fileInfos = this.fileService.getFilesByFileName(audit.file_urls);
    }
  }

  hideDialog() {
    this.editHistoryDialog = false;
    this.auditeeResponseDialog = false;
    this.submitted = false;
  }

  prepareData(audits: BranchFinancialAudit[]) {
    let data: any[] = [];
    let i = 0;
    let dom = document.createElement('div');
    for (const audit of audits) {
      let row: any = {};

      if (audit.case_number) {
        row['Number'] = i + 1;

        if (audit.branch_name) {
          row['Branch Name'] = audit.branch_name;
        }

        row['Case Number'] = audit.audit_case_number;

        row['Amount'] = audit.operationalDescripancyBranch?.amount;

        row['Currency Type'] = audit.operationalDescripancyBranch?.cash_type;

        if (audit.operationalDescripancyBranch?.fcy) {
          row['FCY'] = audit.operationalDescripancyBranch?.fcy;
        }
        row['Category'] =
          audit.operationalDescripancyBranch?.operationalDescripancyCategory?.name;
        row['Account Number'] =
          audit.operationalDescripancyBranch?.account_number;
        row['Currency Type'] = audit.operationalDescripancyBranch?.cash_type;
        row['Account Holder Name'] =
          audit.operationalDescripancyBranch?.acount_holder_name;

        if (audit.finding) {
          dom.innerHTML = '';
          dom.innerHTML = audit.finding.toString();
        }
        row['Finding'] = dom.innerText;

        if (audit.impact) {
          dom.innerHTML = '';
          dom.innerHTML = audit.impact.toString();
        }
        row['Impact'] = dom.innerText;

        if (audit.recommendation) {
          dom.innerHTML = '';
          dom.innerHTML = audit.recommendation.toString();
        }
        row['Recommendation'] = dom.innerText;

        row['Reporting Date'] = this.dateFormat.dateFormatter(
          audit.operationalDescripancyBranch?.transaction_date
        );

        row['Date of Discrepancy'] = this.dateFormat.dateFormatter(
          audit?.finding_date
        );
        data[i++] = row;
      }
    }
    return data;
  }

  exportExcel(): void {
    this.selectedAuditReport = this.selectedAudits
      ? this.selectedAudits
      : this.audits;
    this.audits_to_be_exported = this.prepareData(this.selectedAuditReport);

    this.exportColumns = this.cols_export.map((col) => ({
      title: col.header,
      dataKey: col.header,
    }));

    // this.selectedTitleForReport = 'AFRFMS - Information System Audit Report';

    // let reportData = {
    //   sheet_name: 'Findings Report',
    //   title: 'AFRFMS - Findings Report',
    //   data: this.audits_to_be_exported,
    //   headers: Object.keys(this.audits_to_be_exported[0]),
    // };

    let reportData = {
      sheet_name: this.selectedTitleForReport,
      title: this.selectedTitleForReport,
      data: this.audits_to_be_exported,
      headers: this.exportColumns,
    };

    this.exportService.exportExcelReport(reportData);
  }

  exportPdf() {
    let pdf_allowed_columns: string[];
    pdf_allowed_columns = [
      'Branch Name',
      'Case Number',
      'Amount',
      'Currency Type',
      'Category',
      'Account Number',
      'Finding',
      'Impact',
      'Reporting Date',
    ];

    this.exportColumns = this.cols_export.map((col) => ({
      title: col.header,
      dataKey: col.header,
    }));

    // this.selectedTitleForReport = 'AFRFMS - Information System Audit Report';

    this.selectedAuditReport = this.selectedAudits
      ? this.selectedAudits
      : this.audits;
    this.audits_to_be_exported = this.prepareData(this.selectedAuditReport);

    this.exportService.exportPdf(
      this.exportColumns.filter((pair) =>
        pdf_allowed_columns.includes(pair.title)
      ),
      // pdf_allowed_columns,
      this.audits_to_be_exported,
      this.selectedTitleForReport,
      this.selectedTitleForReport
    );
  }
}
