...
import { RemarkRegionalBranchAuditService } from '../service/remark-regional-branch-audit.service';
import { CompiledRegionalAudit } from 'app/branch_audit/model/compiled-regional-audit';
@Component({
  selector: 'app-remark-regional-branch-audit',
  templateUrl: './remark-regional-branch-audit.component.html',
  styleUrls: ['./remark-regional-branch-audit.component.scss'],
})
export class RemarkRegionalBranchAuditComponent {
  environment = environment;
  submitted: boolean;
  audits: CompiledRegionalAudit[];
  audit = new CompiledRegionalAudit();
  ...
  @Input() compiledRegionalAudit: CompiledRegionalAudit;
  ...
  constructor(
    ...
    private compiledService: RemarkRegionalBranchAuditService,
  ...
  ngOnInit() {
    ...
    this.compiledService.getUserByCategory('BFA').subscribe({
      next: (data) => {
        this.contacts = data;
        this.contacts = this.contacts.filter(
          (val) => val.id !== this.currentUser.id
        );
        this.getUnseenRemarks(this.compiledRegionalAudit);
      },
      error: (error: HttpErrorResponse) => {
  ...
  openRemarkDialog(contact: User) {
    this.seenRemark.compiledRegionalAudit = this.compiledRegionalAudit;
    ...
    if (contact?.unseen_remark) {
      if (contact.unseen_remark > 0) {
        this.compiledService.seenRemark(this.seenRemark).subscribe({
          next: (res) => {},
          error: (error: HttpErrorResponse) => {
            this.messageService.add({
              severity: 'error',
  ...
  sendRemarkEventListener() {
    ...
    this.addRemark.compiledRegionalAudit = this.compiledRegionalAudit;
  ...
  sendRemark(remark: RemarkBranchAudit) {
    this.compiledService.addRemark(remark).subscribe({
      next: () => {},
      error: (error: HttpErrorResponse) => {
        this.submitted = false;
        this.messageService.add({
  ...
  getRemarks(remark: RemarkBranchAudit) {
    this.compiledService.getRemarks(remark).subscribe({
      next: (data) => {
  ...
  getUnseenRemarks(audit: CompiledRegionalAudit) {
    ...
    this.compiledService.getUnseenRemarks(this.unseenRemark).subscribe({
      next: (remarks) => {
