...
@Component({
  selector: 'app-remark-notification-branch',
  templateUrl: './remark-notification-branch.component.html',
  styleUrls: ['./remark-notification-branch.component.css'],
})
export class RemarkNotificationBranchComponent {
  ...
  ngOnInit() {
    ...
    if (this.reviewer) {
      ...
      this.user.category = 'Reviewer';
       this.getBranchRemarkAuditsNotification(this.user);
    } else {
  ...
  getBranchRemarkAuditsNotification(user: User) {
    this.auditService.getBranchRemarkAuditsNotification(user).subscribe(
      (response) => {
