...
import { CashcountdraftService } from '../cashcountdraft/cashcountdraft.service';
@Component({
  selector: 'app-cash-count-creat-edit-branch',
  templateUrl: './cash-count-creat-edit-branch.component.html',
  styleUrls: ['./cash-count-creat-edit-branch.component.css'],
})
export class CashCountCreatEditBranchComponent {
  ...
  getCommonFindings() {
    this.commonFindingService.getFindingsByAuditType('CashCount').subscribe(
      (response) => {
  ...
  getCommonRecommendations() {
    this.recommendationService
      .getRecommondationsByAuditType('CashCount')
      .subscribe(
        (response) => {
  ...
  saveAudit() {
    ...
    if (this.file_uploaded) {
      this.loading = true;
      this.service.saveCashCount(this.auditData).subscribe({
        next: (response) => {
          if (this.auditData.id) {
            this.messageService.add({
              severity: 'success',
              summary: 'Successful',
              detail: 'Cash Count Updated',
              life: 3000,
            });
          } else {
            this.messageService.add({
        ...
        error: (error: HttpErrorResponse) => {
          this.messageService.add({
            ...
            detail:
              error.status == 401
                ? 'You are not permitted to perform this action!'
                : 'Something went wrong while either creating or updating cash count!',
            life: 3000,
          });
          this.loading = false;
        },
  ...
  computeDifference() {
    if (this.auditData.cashcount) {
      if (
        this.auditData.cashcount.actual_cash_count &&
        this.auditData.cashcount.trial_balance
      ) {
        this.auditData.cashcount.difference =
          this.auditData.cashcount.actual_cash_count -
          this.auditData.cashcount.trial_balance;
      }
    }
