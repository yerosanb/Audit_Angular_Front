...
import { CashCountBranchManagerService } from '../service/cash-count-branch-manager.service';

@Component({
  selector: 'app-cash-count-pending-branch-manager',
  templateUrl: './cash-count-pending-branch-manager.component.html',
  styles: [
    `
  ...
  styleUrls: ['./cash-count-pending-branch-manager.component.css'],
})
export class CashCountPendingBranchManagerComponent {
  ...
  constructor(
    ...
    private cashCountBranchManagerService: CashCountBranchManagerService
  ) {}
  ...
  getFindings(branch_audit: any) {
    this.cashCountBranchManagerService.getPendingAudits(branch_audit).subscribe(
      (response) => {
