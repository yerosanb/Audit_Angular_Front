...
import { AtmCardBranchManagerService } from '../service/atm-card-branch-manager.service';
@Component({
  selector: 'app-atm-card-pending-branch-manger',
  templateUrl: './atm-card-pending-branch-manger.component.html',
  styles: [
    `
  ...
  styleUrls: ['./atm-card-pending-branch-manger.component.css'],
})
export class AtmCardPendingBranchMangerComponent {
  ...
  constructor(
    ...
    private atmCardBranchManagerService: AtmCardBranchManagerService,
  ...
  ngOnInit(): void {
    ...
    this.cols = [
      ...
      { field: 'distributed_card', header: 'Distributed Card' },
      { field: 'returned_card', header: 'Returned Card' },
      { field: 'remaining_card', header: 'Remaining Card' },
  ...
  getFindings(branch_audit: any) {
    this.atmCardBranchManagerService.getPendingAudits(branch_audit).subscribe(
      (response) => {
        this.loading = false;
