...
import { AtmCardBranch } from 'app/branch_audit/model/atm_card/atm-card-branch';
...
import { AtmCardAuditorService } from '../services/atm-card-auditor.service';

@Component({
  selector: 'app-atm-card-drafting-finding',
  templateUrl: './atm-card-drafting-finding.component.html',
  styleUrls: ['./atm-card-drafting-finding.component.css'],
})
export class AtmCardDraftingFindingComponent {
  ...
  risk_level_change_tracker = new ChangeTrackerBranchAudit();
  ...
  atmCardBranch = new AtmCardBranch();
  outputAudit: any[] = [];
  isEditData = false;
  ...
  constructor(
    ...
    private atmCardAuditorService: AtmCardAuditorService,
  ...
  getCommonFindings() {
    this.commonFindingService.getCommonFindings().subscribe(
      (response) => {
  ...
  getCommonRecommendations() {
    this.recommendationService.getRecommondations().subscribe(
      (response) => {
  ...
  onChangeRecommendationDropdown(recommendation: String) {
    if (recommendation.includes('New Recommendation')) {
      ...
      this.audit.recommendation = recommendation;
      this.recommendation_change_tracker.change =
        '<strong>Recommendation: </strong>' + recommendation;
      this.recommendation_change_tracker.content_type = 'editor';
      this.change_tracker_branch_audit[2] = this.recommendation_change_tracker;
    }
  }
  ...
  getFindings(auditor_id: number) {
    this.atmCardAuditorService.getAuditsATMOnDrafting(auditor_id).subscribe(
      (response) => {
  ...
  deleteAudit(audit: BranchFinancialAudit) {
    this.confirmationService.confirm({
      ...
      accept: () => {
        this.atmCardAuditorService.deleteBranchAudit(audit.id).subscribe({
          next: (response) => {
  ...
  passAudit(audit: BranchFinancialAudit) {
    this.confirmationService.confirm({
      ...
      icon: 'pi pi-exclamation-triangle',
      accept: () => {
        this.atmCardAuditorService.passAtmAudit(audit.id).subscribe({
          next: (response) => {
            this.audits = this.audits.filter((val) => val.id !== audit.id);
            this.audit = new BranchFinancialAudit();
            this.messageService.add({
  ...
  passSelectedAudits() {
    this.confirmationService.confirm({
      ...
      accept: () => {
        this.atmCardAuditorService
          .passSelectedAtmAudit(this.selectedAudits)
          .subscribe({
            next: (response) => {
              this.audits = this.audits.filter(
                (val) => !this.selectedAudits.includes(val)
  ...
  // saveAudit() {
  //   this.submitted = true;
  //   this.auditor.id = this.storageService.getUser().id;
  ...
  //   this.audit.change_tracker_branch_audit = this.change_tracker_branch_audit;
  //   this.audit.category = this.storageService.getUser().category;
  //   this.audit.atmCardBranch = this.atmCardBranch;
  //   console.log(this.audit.atmCardBranch)

  //   this.atmCardAuditorService.create_Atm(this.audit).subscribe({
  //     next: (response) => {
  //       if (this.audit.id) {
