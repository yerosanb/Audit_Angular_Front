...
@Component({
  selector: 'app-atm-card-create-edit',
  templateUrl: './atm-card-create-edit.component.html',
  styleUrls: ['./atm-card-create-edit.component.css']
})
export class AtmCardCreateEditComponent {
  ...
  getCommonFindings() {
    this.commonFindingService.getFindingsByAuditType("ATMCard").subscribe(
      (response) => {
  ...
  getCommonRecommendations() {
    this.recommendationService.getRecommondationsByAuditType("ATMCard").subscribe(
      (response) => {
  ...
  computeDifference()
  {
    if(this.auditData.atmCardBranch?.issued_card !=null && this.auditData.atmCardBranch?.distributed_card !=null && this.auditData.atmCardBranch?.returned_card != null)
    {
      this.auditData.atmCardBranch.remaining_card = this.auditData.atmCardBranch?.issued_card - this.auditData.atmCardBranch?.distributed_card- this.auditData.atmCardBranch?.returned_card
  ...
  saveAudit() {
    ...
    if (this.file_uploaded) {
      this.loading=true;
      this.atmCardAuditorService.create_Atm(this.auditData).subscribe({
        next: (response) => {
          if (this.auditData.id) {
            this.messageService.add({
              severity: 'success',
              summary: 'Successful',
              detail: 'ATM Card Updated',
              life: 3000,
            });
          } else {
            this.messageService.add({
