...
import { AuditorCashManagementBranchService } from '../service/auditor-cash-management-branch.service';
@Component({
  selector: 'app-cash-management-create-edit-branch',
  templateUrl: './cash-management-create-edit-branch.component.html',
  styleUrls: ['./cash-management-create-edit-branch.component.css'],
})
export class CashManagementCreateEditBranchComponent {
  ...
  constructor(
    ...
    private auditorCashManagementBranchService: AuditorCashManagementBranchService
  ) {}
  ...
  getCommonFindings() {
    this.commonFindingService
      .getFindingsByAuditType("CashManagement")
      .subscribe(
        (response) => {
  ...
  getCommonRecommendations() {
    this.recommendationService
      .getRecommondationsByAuditType("CashManagement")
      .subscribe(
        (response) => {
  ...
  editAudit(passedData: any[]) {
    ...
    this.cash_type_intial = this.auditData.cashManagementBranch?.cash_type;
  ...
  saveAudit() {
    ...
    if (this.file_uploaded) {
      this.loading=true;
      this.auditorCashManagementBranchService
          ...
          next: (response) => {
            if (this.auditData.id) {
              this.messageService.add({
                severity: 'success',
                summary: 'Successful',
                detail: 'Cash Management Updated',
                life: 3000,
              });
            } else {
              this.messageService.add({
                severity: 'success',
                summary: 'Successful',
                detail: 'Cash Management Created',
                life: 3000,
              });
              this.auditData = new BranchFinancialAudit();
            }
          ...
          error: (error: HttpErrorResponse) => {
            ...
            this.messageService.add({
              ...
              detail:
                error.status == 401
                  ? 'You are not permitted to perform this action!'
                  : 'Something went wrong while either creating or updating cash management!',
              life: 3000,
            });
          },
        });
  ...
  computeDifference() {
    if (this.auditData.cashManagementBranch) {
      if (
        this.auditData.cashManagementBranch.average_cash_holding != null &&
        this.auditData.cashManagementBranch.branch_cash_set_limit != null
      ) {
        this.auditData.cashManagementBranch.difference =
          this.auditData.cashManagementBranch.average_cash_holding -
          this.auditData.cashManagementBranch.branch_cash_set_limit;
      }
    }
  }
