...
@Component({
  selector: 'app-asset-liability-creat-edit',
  templateUrl: './asset-liability-creat-edit.component.html',
  styleUrls: ['./asset-liability-creat-edit.component.css'],
})
export class AssetLiabilityCreatEditComponent {
  ...
  constructor(
    ...
    private assetLiabilityAuditorBranchService: AssetLiabilityAuditorBranchService,
  ...
  getCommonFindings() {
    this.commonFindingService
      .getFindingsByAuditType('AssetLiability')
      .subscribe(
        (response) => {
  ...
  getCommonRecommendations() {
    this.recommendationService
      .getRecommondationsByAuditType('AssetLiability')
      .subscribe(
        (response) => {
  ...
  editAudit(passedData: any[]) {
    ...
    this.cash_type_intial = this.auditData.assetLiabilityBranch?.cash_type;
  ...
  saveAudit() {
    ...
    this.auditData.audit_type = 'Asset Liability';
    ...
    if (this.file_uploaded) {
      this.loading=false;
      this.assetLiabilityAuditorBranchService
        .createAssetLiablity(this.auditData)
        .subscribe({
          next: () => {
  ...
  changeActionTaken(editor: any, audit: BranchFinancialAudit) {
    editor.model.document.on('change:data', () => {
      this.action_taken_valid = editor.getData().length > 0 ? true : false;
  ...
  computeDifference() {
    ...
    if (this.auditData.assetLiabilityBranch) {
      if (
        this.auditData.assetLiabilityBranch?.asset_amount &&
        this.auditData.assetLiabilityBranch?.liability_amount
      ) {
        this.auditData.assetLiabilityBranch!.difference =
          this.auditData.assetLiabilityBranch?.asset_amount -
          this.auditData.assetLiabilityBranch?.liability_amount;
      }
    }
