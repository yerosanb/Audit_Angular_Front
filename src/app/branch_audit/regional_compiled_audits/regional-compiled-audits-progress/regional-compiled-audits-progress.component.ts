...
@Component({
  selector: 'app-regional-compiled-audits-progress',
  templateUrl: './regional-compiled-audits-progress.component.html',
  styleUrls: ['./regional-compiled-audits-progress.component.css'],
})
export class RegionalCompiledAuditsProgressComponent {
  ...
  ngOnInit(): void {
    ...
    if(user.roles.includes('ROLE_REGIONALD_BFA'))
    {
      this.user.category = 'REGIONALD';
    }
    ...
    this.compilerService.getSubmittedCompiledFindings(this.user).subscribe(
      (response) => {
