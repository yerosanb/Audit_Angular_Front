...
import { RemarkRegionalBranchAuditService } from './../../../common/remark-branch-audit/service/remark-regional-branch-audit.service';

@Component({
  selector: 'app-compiled-audits-division',
  templateUrl: './compiled-audits-division.component.html',
  styleUrls: ['./compiled-audits-division.component.css'],
})
export class CompiledAuditsDivisionComponent {
  ...
  decompileFinding(audit: CompiledRegionalAudit) {
    this.compiler.id = this.storageService.getUser().id;
    ...
    this.confirmationService.confirm({
      ...
      accept: () => {
        this.compilerService
          .decompileFinding(this.compiledRegionalAudit)
          .subscribe({
            next: (response) => {
              this.compiledRegionalAudits = this.compiledRegionalAudits.filter(
                (val) => val.id !== audit.id
  ...
  submitFinding(audit: CompiledRegionalAudit) {
    this.compiler.id = this.storageService.getUser().id;
    this.compiledRegionalAudit = audit;
    this.compiledRegionalAudit.compiler = this.compiler;
    this.confirmationService.confirm({
      ...
      accept: () => {
        this.compilerService
          .submitCompiledFindings(this.compiledRegionalAudit)
          .subscribe({
            next: (response) => {
              this.compiledRegionalAudits = this.compiledRegionalAudits.filter(
                (val) => val.id !== audit.id
