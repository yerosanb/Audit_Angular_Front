...
@Component({
  selector: 'app-incomplete-account-passed-branch',
  templateUrl: './incomplete-account-passed-branch.component.html',
  styles: [
    `
  ...
  styleUrls: ['./incomplete-account-passed-branch.component.css']
})

export class IncompleteAccountPassedBranchComponent {
  ...
  ngOnInit() {
    ...
    this.cols = [
      ...
      { field: 'account_type', header: 'Risk Level' },
  ...
  getFindings(auditor_id: number) {
    this.auditService.getPassedAudits(auditor_id).subscribe(
      (response) => {
