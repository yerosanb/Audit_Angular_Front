...
@Component({
  selector: 'app-general-observation-pending-branch',
  templateUrl: './general-observation-pending-branch.component.html',
  styles: [
    `
  ...
  styleUrls: ['./general-observation-pending-branch.component.css'],
})
export class GeneralObservationPendingBranchComponent {
  ...
  ngOnInit() {
    ...
    this.getPendingGeneralObservation(this.user_type);
  ...
  reviewSingleGeneralObservation(audit: BranchFinancialAudit) {
  ...
  getPendingGeneralObservation(category: any) {
    this.reviewerService.getPendingGeneralObservation(category).subscribe(
      (response) => {
  ...
  reviewSelectedGeneralObservation() {
    this.user_type.id = this.storageService.getUser().id;
  ...
  onDataLoad(data: any) {
    if (data[1]) {
      let auditor_id = this.storageService.getUser().id;
      this.getPendingGeneralObservation(auditor_id);
