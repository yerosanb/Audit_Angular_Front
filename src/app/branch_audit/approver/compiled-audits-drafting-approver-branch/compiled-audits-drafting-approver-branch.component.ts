...
@Component({
  selector: 'app-compiled-audits-drafting-approver-branch',
  templateUrl: './compiled-audits-drafting-approver-branch.component.html',
  ...
  styleUrls: ['./compiled-audits-drafting-approver-branch.component.scss'],
})
export class CompiledAuditsDraftingApproverBranchComponent {
  ...
  passAudits: CompiledRegionalAudit[] = [];
  audit_edit_history: CompiledRegionalAudit[];
  ...
  constructor(
    private approverService: ApproverBranchService,
  ...
  ngOnInit(): void {
    ...
    this.approverService.getDraftedAudits(user.id).subscribe(
      (response) => {
  ...
  openAuditDetailDialog(compiledBranchAudit: CompiledBranchAudit) {
    ...
    this.audit_type = compiledBranchAudit.audit_type
  ...
  approveSelectedFindings() {
    this.approver.id = this.storageService.getUser().id;
    this.selectedAudits[0].approver = this.approver;
    this.confirmationService.confirm({
      message: 'Are you sure you want to approve selected compiled audits ?',
      header: 'Confirm',
      icon: 'pi pi-exclamation-triangle',
      accept: () => {
        this.approverService.approveAudit(this.selectedAudits).subscribe({
          next: (response) => {
            this.compiledRegionalAudits = this.compiledRegionalAudits.filter(
            ...
            this.messageService.add({
              severity: 'success',
              summary: 'Successful',
              detail: 'Compiled Audits Approved',
              life: 3000,
            });
          },
          error: (error: HttpErrorResponse) => {
            ...
            this.messageService.add({
              ...
              summary:
                error.status == 401
                  ? 'You are not permitted to perform this action!'
                  : 'Something went wrong while approving compiled findings!',
              detail: '',
            });
          },
        });
  ...
  approveFinding(audit: CompiledRegionalAudit) {
    this.approver.id = this.storageService.getUser().id;
    this.compiledRegionalAudit = audit;
    this.compiledRegionalAudit.approver = this.approver;
    this.passAudits.push(this.compiledRegionalAudit);
    this.confirmationService.confirm({
      message: 'Are you sure you want to approve the given audit ?',
      header: 'Confirm',
      icon: 'pi pi-exclamation-triangle',
      accept: () => {
        this.approverService.approveAudit(this.passAudits).subscribe({
          next: (response) => {
            this.compiledRegionalAudits = this.compiledRegionalAudits.filter(
              (val) => val.id !== audit.id
            );
            this.compiledRegionalAudit = new CompiledRegionalAudit();
            this.messageService.add({
              severity: 'success',
              summary: 'Successful',
              detail: 'Audit Approved',
              life: 3000,
            });
          },
          error: (error: HttpErrorResponse) => {
            ...
            this.messageService.add({
              ...
              summary:
                error.status == 401
                  ? 'You are not permitted to perform this action!'
                  : 'Something went wrong while approving compiled finding!',
              detail: '',
            });
          },
        });
  ...
  editAudit(audit: CompiledRegionalAudit) {
    this.compiledRegionalAudit = { ...audit };
  ...
  saveAudit() {
    this.approver.id = this.storageService.getUser().id;
    this.approver.category = this.storageService.getUser().category;
    this.changer.id = this.storageService.getUser().id;
    this.compiledRegionalAudit.change_tracker_branch_audit =
      this.changeTrackerBranchAudit;
    this.compiledRegionalAudit.approver = this.approver;

    this.approverService.updateFinding(this.compiledRegionalAudit).subscribe({
      next: (response) => {
        if (this.compiledRegionalAudit.id) {
          this.compiledRegionalAudits[
            this.findIndexById(this.compiledRegionalAudit.id)
          ] = this.compiledRegionalAudit;
          this.messageService.add({
            severity: 'success',
            summary: 'Successful',
      ...
      error: (error: HttpErrorResponse) => {
        ...
        this.messageService.add({
          ...
          detail:
            error.status == 401
              ? 'You are not permitted to perform this action!'
              : 'Something went wrong while updating finding!',
          life: 3000,
        });
      },
    });
  ...
  findIndexById(id: Number): number {
    let index = -1;
    for (let i = 0; i < this.compiledRegionalAudits.length; i++) {
      if (this.compiledRegionalAudits[i].id === id) {
        index = i;
        break;
      }
    }
  ...
  changeFinding(editor: any, audit: CompiledRegionalAudit) {
    editor.model.document.on('change:data', () => {
      if (audit.finding) {
        if (audit.finding != editor.getData()) {
          ...
          this.changeTrackerBranchAudit[0] = this.finding_change_tracker;
        } else {
          this.finding_change_tracker = new ChangeTrackerBranchAudit();
          this.changeTrackerBranchAudit[0] = {};
        }
      }
    });
  }

  changeImpact(editor: any, audit: CompiledRegionalAudit) {
    editor.model.document.on('change:data', () => {
      if (audit.impact) {
        if (audit.impact != editor.getData()) {
          ...
          this.changeTrackerBranchAudit[1] = this.impact_change_tracker;
        } else {
          this.impact_change_tracker = new ChangeTrackerBranchAudit();
          this.changeTrackerBranchAudit[1] = {};
        }
      }
    });
  }

  changeRecommendation(editor: any, audit: CompiledRegionalAudit) {
    editor.model.document.on('change:data', () => {
      if (audit.recommendation)
        if (audit.recommendation != editor.getData()) {
          ...
          this.changeTrackerBranchAudit[2] = this.recommendation_change_tracker;
        } else {
          this.recommendation_change_tracker = new ChangeTrackerBranchAudit();
          this.changeTrackerBranchAudit[2] = {};
        }
    });
  }
  ...
  getApproverEditHistory(
    ...
    this.audit_edit_history = this.compiledRegionalAudits.filter(
      (audit) => audit.id == id
    );
  ...
  exportExcel() {
    import('xlsx').then((xlsx) => {
      ...
      this.saveAsExcelFile(excelBuffer, 'Drafted Audits');
    });
  }
