...
@Component({
  selector: 'app-compiled-approved-audits-branch',
  templateUrl: './compiled-approved-audits-branch.component.html',
  styles: [
    `
  ...
  styleUrls: ['./compiled-approved-audits-branch.component.scss'],
})
export class CompiledApprovedAuditsBranchComponent {
  ...
  ngOnInit(): void {
    ...
    this.approverService.getApprovedAudits(user.id).subscribe(
      (response) => {
  ...
  unapproveSelectedFindings() {
    this.approver.id = this.storageService.getUser().id;
    this.selectedAudits[0].approver = this.approver;
    this.confirmationService.confirm({
      message:
        'Are you sure you want to cancel approvement for selected audits ?',
      header: 'Confirm',
      icon: 'pi pi-exclamation-triangle',
      accept: () => {
        this.approverService.unapproveAudit(this.selectedAudits).subscribe({
          next: (response) => {
            ...
            this.messageService.add({
              severity: 'success',
              summary: 'Successful',
              detail: 'Selected Audits Unapproved',
              life: 3000,
            });
          },
          error: (error: HttpErrorResponse) => {
            ...
            this.messageService.add({
              ...
              summary:
                error.status == 401
                  ? 'You are not permitted to perform this action!'
                  : 'Something went wrong while cancelling approval for selected findings!',
              detail: '',
            });
          },
        });
  ...
  closeSelectedFindings() {
    this.approver.id = this.storageService.getUser().id;
    this.selectedAudits[0].approver = this.approver;
    this.confirmationService.confirm({
      message: 'Are you sure you want to close case for selected audits ?',
      header: 'Confirm',
      icon: 'pi pi-exclamation-triangle',
      accept: () => {
        this.approverService.closeAudit(this.selectedAudits).subscribe({
          next: (response) => {
            this.compiledRegionalAudits = this.compiledRegionalAudits.filter(
              (val) => !this.selectedAudits.includes(val)
            );
            ...
            this.messageService.add({
              severity: 'success',
              summary: 'Successful',
              detail: 'Selected Audits Case closed',
              life: 3000,
            });
          },
          error: (error: HttpErrorResponse) => {
            ...
            this.messageService.add({
              ...
              summary:
                error.status == 401
                  ? 'You are not permitted to perform this action!'
                  : 'Something went wrong while closing cases for selected findings!',
              detail: '',
            });
          },
        });
  ...
  unapproveFinding(audit: CompiledRegionalAudit) {
    this.approver.id = this.storageService.getUser().id;
    ...
    this.confirmationService.confirm({
      message: 'Are you sure you want to cancel approval for the given audit ?',
      header: 'Confirm',
      icon: 'pi pi-exclamation-triangle',
      accept: () => {
        this.approverService.unapproveAudit(this.passAudits).subscribe({
          next: (response) => {
            this.compiledRegionalAudits = this.compiledRegionalAudits.filter(
              (val) => val.id !== audit.id
            );
          ...
          error: (error: HttpErrorResponse) => {
            ...
            this.messageService.add({
              ...
              summary:
                error.status == 401
                  ? 'You are not permitted to perform this action!'
                  : 'Something went wrong while cancelling approval for selected finding!',
              detail: '',
            });
          },
        });
  ...
  exportExcel() {
    import('xlsx').then((xlsx) => {
      ...
      this.saveAsExcelFile(excelBuffer, 'Approved Audits');
    });
  }
