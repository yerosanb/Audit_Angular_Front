...
import { AbnormalBalanceAuditorBranchService } from '../service/abnormal-balance-auditor-branch.service';


@Component({
  selector: 'app-abnormal-balance-drafting-branch',
  templateUrl: './abnormal-balance-drafting-branch.component.html',
  styles: [
    `
  ...
  styleUrls: ['./abnormal-balance-drafting-branch.component.css']
})
export class AbnormalBalanceDraftingBranchComponent {
  ...
  constructor(
    ...
    private abnormalBalanAuditorService: AbnormalBalanceAuditorBranchService,
  ...
  getFindings(auditor_id: number) {
    this.abnormalBalanAuditorService.getDraftedAbnormalBalance(auditor_id).subscribe(
      (response) => {
  ...
  deleteAudit(audit: BranchFinancialAudit) {
    this.confirmationService.confirm({
      ...
      accept: () => {
        this.abnormalBalanAuditorService.deleteAbnormalBalance(audit).subscribe({
          next: (response) => {
  ...
  passAudit(audit: BranchFinancialAudit) {
    this.confirmationService.confirm({
      message: 'Are you sure you want to pass ' + audit.case_number + ' to reviewer?',
      header: 'Confirm',
      icon: 'pi pi-exclamation-triangle',
      accept: () => {
        this.abnormalBalanAuditorService.passAbnormalBalance(audit).subscribe({
          next: (response) => {
            this.audits = this.audits.filter((val) => val.id !== audit.id);
            this.audit = new BranchFinancialAudit();
            this.messageService.add({
  ...
  deleteSelectedAudits() {
    this.confirmationService.confirm({
      ...
      icon: 'pi pi-exclamation-triangle',
      accept: () => {
        this.abnormalBalanAuditorService
          .deleteSelectedAbnormalBalance(this.selectedAudits)
          .subscribe({
            next: (response) => {
  ...
  passSelectedAudits() {
    ...
    this.confirmationService.confirm({
      ...
      icon: 'pi pi-exclamation-triangle',
      accept: () => {
        this.abnormalBalanAuditorService.passSelectedAbnormalBalance(this.selectedAudits).subscribe({
          next: (response) => {
            this.audits = this.audits.filter(
              (val) => !this.selectedAudits.includes(val)
            );
