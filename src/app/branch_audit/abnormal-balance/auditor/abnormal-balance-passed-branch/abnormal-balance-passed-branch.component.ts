...
@Component({
  selector: 'app-abnormal-balance-passed-branch',
  templateUrl: './abnormal-balance-passed-branch.component.html',
  styles: [
    `
  ...
  styleUrls: ['./abnormal-balance-passed-branch.component.css'],
})
export class AbnormalBalancePassedBranchComponent {
  ...
  constructor(
    ...
    private abnormalBalanceAuditorService: AbnormalBalanceAuditorBranchService,
  ...
  getFindings(auditor_id: number) {
    this.abnormalBalanceAuditorService.getPassedAbnormalBalance(auditor_id).subscribe(
      (response) => {
  ...
  backAudit(audit: BranchFinancialAudit) {
    this.confirmationService.confirm({
      ...
      accept: () => {
        this.abnormalBalanceAuditorService
          .backPassedAbnormalBalance(audit)
          .subscribe({
            next: (response) => {
  ...
  backSelectedAudits() {
    ...
    this.confirmationService.confirm({
      ...
      icon: 'pi pi-exclamation-triangle',
      accept: () => {
        this.abnormalBalanceAuditorService.backSelectedPassedAbnormalBalance(this.selectedAudits).subscribe({
          next: (response) => {
            ...
            this.messageService.add({
              severity: 'success',
              summary: 'Successful',
              detail: 'Selected Audits Backe to Drafted State',
              life: 3000,
            });
          },
          error: (error: HttpErrorResponse) => {
    ...
    this.messageService.add({
      severity: 'error',
      summary:"Please choose audits that haven't had any reviewer actions taken.",
      detail: '',
      life: 6000,
    });
  }
  ...
  onDataChange(data: BranchFinancialAudit[]) {
    if (data) {
      for (const dt of data) {
        if (dt.id) {
          this.audits[this.findIndexById(dt.id)] = dt;
        }
      }
    }
